...
DiriPlotData <- function ( gridsize =100 , alpha ) {
  ...
  for(x in 1: gridsize ) {
    for (y in 1: gridsize ) {
      ...
      if (( gridaxis [x] + gridaxis [y]) < 1) dens [x,y] <-
...
DiriPersp <- function ( alph =c(2 ,3 ,2) , ...) {
  ...
  persp ( diridata$gridline, diridata$gridline, diridata$values, zlim =c(-3,3), theta = 310, 
        expand = 0.5, col ="lightblue", ticktype ="detailed", xlab ="X", ylab ="Y", zlab ="density", ...)
  title ( main = bquote ( list ( alpha [1]==.( alph [1]) , alpha [2]==.( alph [2]) , alpha [3]==.( alph [3]) )))
  }

par( mfrow =c(3 ,2))
dev.new()
DiriPersp (c(132.9534 ,132.9534 ,132.9534) ,phi = 50)
...
sample_theta <- rdirichlet(n=1, alpha=rep(132,15))
entropy.plugin(sample_theta)
